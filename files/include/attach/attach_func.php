...
//binary check if check is in rules
function attach_rules($rules = 0, $check = 1)
{
	if ($rules & $check && $rules != 0)
		return true;
	else
		return false;
}
...
function attach_icon($extension)
{
	...
	if (count($attach_icons) == 0 && strlen($pun_config['attach_icon_extension']) != 0)
	...
	if ($pun_user['show_img'] == 0 || $pun_config['attach_use_icon'] == 0)
		return '';

	// if extension exist, assign that one to the url instead of the default
	if (array_key_exists($extension,$attach_icons))
		$icon_url = $pun_config['attach_icon_folder'].$attach_icons[$extension];
